...
using AndersonAPI.Application.Regions;
using AndersonAPI.Application.Regions.CreateRegion;
using AndersonAPI.Application.Regions.DeleteRegion;
using AndersonAPI.Application.Regions.GetRegionById;
using AndersonAPI.Application.Regions.GetRegions;
using AndersonAPI.Application.Regions.SetStateRegion;
using AndersonAPI.Application.Regions.UpdateRegion;
...
namespace AndersonAPI.Api.Controllers
{
    ...
    public class RegionsController : ControllerBase
    {
        ...
        public async Task<ActionResult<JsonResponse<Guid>>> CreateRegion(
            [FromBody] CreateRegionCommand command,
            CancellationToken cancellationToken = default)
        {
            var result = await _mediator.Send(command, cancellationToken);
            return CreatedAtAction(nameof(GetRegionById), new { id = result }, new JsonResponse<Guid>(result));
        }
        ...
        [ProducesResponseType(typeof(ProblemDetails), StatusCodes.Status500InternalServerError)]
        public async Task<ActionResult> DeleteRegion([FromRoute] Guid id, CancellationToken cancellationToken = default)
        {
        ...
        /// <response code="400">One or more validation errors have occurred.</response>
        /// <response code="404">One or more entities could not be found with the provided parameters.</response>
        [HttpPut("api/regions/{id}/set-state")]
        [ProducesResponseType(StatusCodes.Status204NoContent)]
        [ProducesResponseType(StatusCodes.Status400BadRequest)]
        [ProducesResponseType(StatusCodes.Status404NotFound)]
        [ProducesResponseType(typeof(ProblemDetails), StatusCodes.Status500InternalServerError)]
        public async Task<ActionResult> SetStateRegion(
            [FromRoute] Guid id,
            [FromBody] SetStateRegionCommand command,
            CancellationToken cancellationToken = default)
        {
